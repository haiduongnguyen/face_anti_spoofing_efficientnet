...
def check_bad_image(data_path): 

    bad_list=[]

    subdir_list = os.listdir(data_path)             # create a list of the sub directories in the directory ie train or test

    for d in subdir_list:                           # iterate through the sub directories train and test

        dpath=os.path.join (data_path, d)           # create path to sub directory
